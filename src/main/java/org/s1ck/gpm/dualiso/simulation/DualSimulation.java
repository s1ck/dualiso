...
import static org.s1ck.gpm.dualiso.utils.ArrayUtils.intersect;
import static org.s1ck.gpm.dualiso.utils.ArrayUtils.union;
      ...
      for (int u_Q = 0; u_Q < q.getSize(); u_Q++) {
        ...
        for (int v_Q : q.getNeighbors(u_Q)) {
          ...
          for (int u_G : candidates[u_Q]) {
            int[] intersect = intersect(g.getNeighbors(u_G), candidates[v_Q]);
            ...
            v_Q_candidates = union(v_Q_candidates, intersect);
          }
          ...
          candidates[v_Q] = intersect(candidates[v_Q], v_Q_candidates);
        }
      }
    }
    return candidates;
